...
public class StreamProblemSet2 {
  ...
  public static void filterPrime() {
    Predicate<Integer> notPrime = n -> n > 1 && IntStream.rangeClosed(2, (int) Math.sqrt(n))
        .anyMatch(div -> n % div == 0);
    List<Integer> integers = List.of(2, 5, 3, 9, 10, 16, 7, 15);
    List<Integer> nonPrimes = integers.stream().filter(notPrime).toList();
  ...
  public static void mergeLists() {
    ...
    List<Integer> merge = Stream.concat(integers1.stream(), integers2.stream()).distinct().toList();
  ...
  public static void firstNonRepeatedCharacter() {
    ...
    Character res = word.chars().mapToObj(c -> Character.toLowerCase((char) c))
  ...
  public static void longestSubsequence() {
    List<Integer> numbers = Arrays.asList(10, 22, 9, 30, 21, 30, 41, 60, 80);
    List<Integer> longestIncreasingSubsequence = numbers.stream()
        .collect(toMap(num -> num, num ->
